...
<v-tooltip bottom content-class="bottom" v-else>
    ...
    <span>{{$t('issues.restore_report_issue')}}</span>
        ...
        <v-navigation-drawer v-model="menuDrawer" :mini-variant="miniVariant" app fixed floating class="drawer-th pt-2"
            ...
            <perfect-scrollbar style="height:100%">
                ...
                <v-list shaped dark>
                    <var :set="enabledMenus = menu.filter(x => !x.admin)"
                        v-if="$store.state.auth.loggedUser.role != 'admin'">
                    </var>
                    ...
                    <div v-for="(link, i) in enabledMenus" :key="i">
                        ...
                        <v-list-group v-else :key="link.text">
                            ...
                            <v-list-item v-for="sublink in link.subLinks" :to="sublink.to" :key="sublink.text"
                                active-class="primary-light" dense :class="!miniVariant ? 'pl-8 mr-6' : 'pl-5'">
                                <v-tooltip right>
                                    <template v-slot:activator="{ on }">
                                        <v-list-item-icon v-on="on">
                                            <v-icon :class="sublink.iconClass" :small="miniVariant">{{ sublink.icon
                                                }}</v-icon>
                                        </v-list-item-icon>
                                    </template>
        ...
        <IssuesNewIssueDialog v-model="newIssueDialog" :user="user" :type="issueType" @minimize="minimizeNewIssue()"
            @cancel="cancelNewIssue()" />
