                ...
                <v-card-text>
                    <v-form v-model="editPostForm">
                        <v-row>
                            ...
                            <v-col>
                                <v-text-field dense label="Etiqueta" v-model="post.author" outlined color="secondary">
                                </v-text-field>
                            </v-col>
                        </v-row>
                        <v-text-field dense label="Título" v-model="post.title" :rules="rules.required" required outlined
                        ...
                        <v-row>
                            ...
                            <v-img :src="$config.storage + 'posts%2F' + post.img + '?alt=media'" cover height="150"  v-if="post.img && post.img != ''"></v-img>
...
export default {
    ...
    methods: {
        ...
        async updateOnHome(item, value) {
            ...
            await this.$fire.firestore.collection('posts').doc(item.id).update({
                onHome: value
            }).then(result => {
                this.listPosts()
                this.$snackbar.show({
